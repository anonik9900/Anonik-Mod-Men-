...
void process_main_menu()
{
	...
	std::string caption = "Anonik Mod Menu";
	...
	while (true)
	{
		...
		{
			// draw menu
			draw_menu_line(caption, lineWidth, 7.9, 14.0, 4.0, 4.0, false, true);   //If you use this design in a public menu, please credit me, The Crusader, for the design :)
			for (int i = 0; i < lineCount; i++)
				if (i != activeLineIndexMain)
					draw_menu_line(lineCaption[i], lineWidth, 4.0, 60.0 + i * 22.8, 4.0, 9.0, false, false);

			draw_menu_line(lineCaption[activeLineIndexMain], lineWidth + 0.0, 2.0, 60.0 + activeLineIndexMain * 22.9, 4.0, 7.0, true, false);
